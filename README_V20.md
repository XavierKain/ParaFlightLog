...
v20 is a simplified, hardened rebuild of the app around its core: **track flights from the Apple Watch, browse them with clean stats on the iPhone, never lose data**. English-only UI. It installs **side by side** with your current app (new bundle id `com.xavierkain.ParaFlightLog2`, display name "ParaFlightLog 2"), so your production data is never at risk while testing.
...
3. **iCloud**: the entitlements file (`ParaFlightLog/ParaFlightLog.entitlements`) already declares CloudKit with container `iCloud.com.xavierkain.ParaFlightLog2`. In Signing & Capabilities, Xcode should pick it up; if the container is missing, add the iCloud capability once so Xcode creates the container on the developer portal.
